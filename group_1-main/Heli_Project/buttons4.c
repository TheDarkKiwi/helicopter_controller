...
initButtons (void)
{
    ...
    SysCtlPeripheralEnable(SWITCH_BUT_PERIPH);
    GPIOPinTypeGPIOInput(SWITCH_BUT_PORT_BASE, SWITCH1_BUT_PIN);
    GPIOPadConfigSet(SWITCH_BUT_PORT_BASE, SWITCH1_BUT_PIN, GPIO_STRENGTH_2MA,
                     GPIO_PIN_TYPE_STD_WPD);
    but_normal[SWITCH1] = SWITCH1_BUT_NORMAL;

    // SWITCH2 button (active HIGH)
    GPIOPinTypeGPIOInput(SWITCH_BUT_PORT_BASE, SWITCH2_BUT_PIN);
    GPIOPadConfigSet(SWITCH_BUT_PORT_BASE, SWITCH2_BUT_PIN, GPIO_STRENGTH_2MA,
                     GPIO_PIN_TYPE_STD_WPD);
    but_normal[SWITCH2] = SWITCH1_BUT_NORMAL;
...
updateButtons (void)
{
    ...
    but_value[SWITCH1] = (GPIOPinRead (SWITCH_BUT_PORT_BASE, SWITCH1_BUT_PIN) == SWITCH1_BUT_PIN);
    but_value[SWITCH2] = (GPIOPinRead (SWITCH_BUT_PORT_BASE, SWITCH2_BUT_PIN) == SWITCH2_BUT_PIN);
...
void initResetInt(void) {
    //Initialize the reset interupt for switch2
    GPIOIntRegister(SWITCH_BUT_PORT_BASE, resetIntHandler);
    GPIOIntTypeSet(SWITCH_BUT_PORT_BASE, SWITCH2_BUT_PIN, GPIO_FALLING_EDGE);
    GPIOIntEnable(SWITCH_BUT_PORT_BASE, SWITCH2_BUT_PIN);
}

void resetIntHandler(void) {
    //Interupt handler for the reset switch
    SysCtlReset();
    GPIOIntClear(SWITCH_BUT_PORT_BASE, SWITCH2_BUT_PIN);
}
